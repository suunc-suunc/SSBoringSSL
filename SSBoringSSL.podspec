Pod::Spec.new do |spec|
  spec.name         = "SSBoringSSL"
  version = "1.0.0"
  spec.version      = version
  spec.summary      = "SSBoringSSL - SSL library."
  spec.description  = <<-DESC
        SSBoringSSL - SSL library for iOS, macOS, tvOS and watchOS
                   DESC
  spec.homepage = 'https://github.com/suunc-suunc/boringssl'
  spec.license  = { :type => 'Mixed', :file => 'LICENSE' }
  spec.author             = { "SỬU NGUYỄN CÔNG" => "suunc.suunc@gmail.com" }
  spec.source = {
    :git => 'https://github.com/suunc-suunc/boringssl.git',
  ...
  spec.header_mappings_dir = 'src/include/openssl'
  ...
  spec.module_map = 'src/include/openssl/BoringSSL.modulemap'
  ...
  spec.prepare_command = <<-END_OF_COMMAND
    ...
    find . -type f \\( -path '*.h' -or -path '*.cc' -or -path '*.c' \\) -print0 | xargs -0 -L1 sed -E -i'.grpc_back' 'spec;#include <openssl/;#include <openssl_grpc/;g'
    # Include of boringssl_prefix_symbols.h does not follow Xcode import style. We add the package
    # name here so that Xcode knows where to find it.
    find . -type f \\( -path '*.h' -or -path '*.cc' -or -path '*.c' \\) -print0 | xargs -0 -L1 sed -E -i'.grpc_back' 'spec;#include <boringssl_prefix_symbols.h>;#include <openssl_grpc/boringssl_prefix_symbols.h>;g'
  END_OF_COMMAND
end
